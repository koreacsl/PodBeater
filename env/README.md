...
### PodBeater environment setup order
- Tu (PodBeater_same / PodBeater_random):

  First deploy the initial pods produced by `initial_pods_Tu.py`, then deploy the benign pods produced by `benign_pods_Tu.py`.

```bash
# deploy initial pods first
kubectl apply -f <path/to/initial_pods_Tu_yaml_or_directory>

# then deploy benign pods
kubectl apply -f <path/to/benign_pods_Tu_yaml_or_directory>
```

These initial pods serve as anchors, ensuring that the affinity conditions of subsequently deployed benign pods can be satisfied.



- Tr (PodBeater_v3 / PodBeater_v5):

  Deploy the benign pods generated by `benign_pods_Tr.py`.
